...
class InfoController extends BackendController
{
	...
	public function postRegist(Request $request){
		...
		if($data['info_type'] == 1){
			...
			if ( Input::hasFile('info3_img') ){
				$info3_img =Input::file('info3_img');
				$random = rand(time(), '9999');
				$fn_info3_img = $random . '_' . $info3_img->etClientOriginalName();
				Image::make($info3_img)-save(public_path('upload/infos/images/' . $fn_info3_img ));
				$data['info3_img']= '/upload/infos/images/' . $fn_info3_img;
			}else{
				if(!empty(Input::get('info3_img_old'))){
					$data['info3_img']= Input::get('info3_img_old');
				}
			}			
			...
			if (Input::hasFile('info3_file')) {
				...
				$_info3_ext = $info3_file->getClientOriginalExtension();
	...
	public function postEdit($id){
		...
		if($data['info_type'] == 1){
			...
			$data['info3_contents'] = Input::get('info3_contents');
			//Image
			if(!empty(Input::get('info3_img_radio')) && Input::get('info3_img_radio') == 2){	
				if (Input::hasFile('info3_img')) {
					$info3_img = Input::file('info3_img');
					$random = rand(time(), '9999');
					$fn_info3_img = $random1. '_' . $info3_img->getClientOriginalName();
					Image::make($info3_img)->save(public_path('upload/infos/images/' . $fn_info3_img ));
					$data['info3_img'] = '/upload/infos/images/' . $fn_info3_img;
				}else{
					if(!empty(Input::get('info3_img_old')) && Input::get('info3_img_radio') == 1)
						$data['info3_img'] = Input::get('info3_img_old');
					elseif(empty(Input::get('info3_img')) && Input::get('info3_img_radio') == 2)
						$data['info3_img'] = NULL;
				}
			}elseif(!empty(Input::get('info3_img_radio')) && Input::get('info3_img_radio') == 3){
				$data['info3_img'] = NULL;
			}else{
				if(!empty(Input::get('info3_img_old')))
					$data['info3_img'] = Input::get('info3_img_old');
			...
			//File
			if(!empty(Input::get('info3_file_radio')) && Input::get('info3_file_radio') == 2){
				if (Input::hasFile('info3_file')) {
					$info3_file = Input::file('info3_file');
					$info3_path_name = $info3_file->getPathName();
					$info3_ext= $info3_file->getClientOriginalExtension();
					$name_info3 = $info3_file->getClientOriginalName();
					$arr_info3 = explode('.', $name_info3);
					$info3_txt_name = $arr_info3[0];
					$fn_info3_fle1 = $info3_txt_name . '_' . rand(time(), '9999') . '.' . $info3_ext;
					move_uploaded_file($info3_path_name, base_path() . '/public/upload/infos/files/' . $fn_info3_file);
					$data['info3_file'] = '/upload/infos/files/' . $fn_info3_fle;
				}else{
					if(!empty(Input::get('info3_file_old')) && Input::get('info3_file_radio') == 1)
						$data['info3_file'] = Input::get('info3_file_old');
					elseif(empty(Input::get('info3_file')) && Input::get('info3_file_radio') == 2)
						$data['info3_file'] = NULL;
				}
			}elseif(!empty(Input::get('info3_file_radio')) && Input::get('info3_file_radio') == 3){
				$data['info3_file'] = NULL;
			}else{
				if(!empty(Input::get('info3_file_old')))
					$data['info3_file'] = Input::get('info3_file_old');
